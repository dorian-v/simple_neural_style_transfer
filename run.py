...
def get_content_image_activations(model, content_image):
...
def get_style_image_activations(model, STYLE_LAYERS, style_image):
...
if __name__ == "__main__":
    ...
    content_image = scipy.misc.imread(sys.argv[1])
    content_image = reshape_and_normalize_image(content_image)
    style_image = scipy.misc.imread(sys.argv[2])
    ...
    a_C, a_G = get_content_image_activations(model, content_image)

    # Set a_S to be the hidden layers activations from the layers we have selected with style image as input
    a_S = get_style_image_activations(model, STYLE_LAYERS, style_image)
